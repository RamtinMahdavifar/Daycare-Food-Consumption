...
Widget listedInst(BuildContext context, String name, String address) {
  return Card(
      child: ListTile(
          onTap: () {
            Config.log.i("navigating to institution with name: " +
                name +
                " and address: " +
                address);
...
Widget searchInstitution() {
  ...
  return Flexible(
    child: Card(
        ...
        elevation: 2,
        child: ListTile(
            leading: icon, onTap: () {}, title: Text("Search Institutions"))),
  );
}
      ...
      child: StreamBuilder<Event>(
          ...
          builder: (BuildContext context, AsyncSnapshot<Event> snapshot) {
            ...
            if (snapshot.hasError) {
              Config.log.e(
                  "errors occurred while reading institutions from the database on institution page, error: " +
              ...
              switch (snapshot.connectionState) {
                ...
                case ConnectionState.waiting:
                  Config.log.i(
                      "connection state waiting when reading institutions from the database");
                ...
                case ConnectionState.active:
                  ...
                  if (researchGroupSnapshot.value == null) {
                    ...
                    Config.log.w(
                        "DataSnapshot has null value when reading institutions from the database, ie no institutions are read");
                    return Center(
                        child: Text(
                            "No Institutions Have Yet Been Created, be the First to Create One Above",
                            style: TextStyle(fontSize: 28.0)));
                  } else {
                    // otherwise we do have institutions on the database, display these
                    Config.log.i(
                        "Institutions present in DataSnapshot, displaying these");
                    Map<dynamic, dynamic> snapshotValueMap =
                        researchGroupSnapshot.value as Map<dynamic, dynamic>;
                    String encodedMap = jsonEncode(snapshotValueMap);
                    Map<String, dynamic> researchGroupJSON =
                    ...
                    Map<String, dynamic> institutionsMap =
                        researchGroupJSON["_institutionsMap"];
                    ...
                    List<InstitutionInfo> existingInstitutions = institutionsMap
                        .values
                        .map((institutionJSON) =>
                            InstitutionInfo.fromJSON(institutionJSON))
                        .toList();
                    ...
                    children = existingInstitutions
                        .map((institutionInfo) => listedInst(
                            context,
