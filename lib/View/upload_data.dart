...
class _UploadDataState extends State<UploadData> {
  ...
  void _onImageButtonPressed(ImageSource source,
    ...
    Config.log.i(
        "image button pressed, waiting for user to choose image from source: " +
            source.toString());
    ...
    await _displayPickImageDialog(context!, () async {
      try {
        ...
        Config.log
            .e("error occurred while picking image, error: " + e.toString());
        setState(() {
          _pickImageError = e;
        });
      }
  ...
  Widget submitImage() {
    return ElevatedButton(
        ...
        onPressed: () {
          ...
          Config.log.i("writing the image with path: " +
              this._imageFileList![0].path +
              " to the database");
          ...
          Database().writeMealToDatabase(
              ResearchGroupInfo("testResearchGroupName"), newMeal);

          // after the new meal has been submitted, clear the data in the fields so
          // the user can submit other meals
          Config.log.i(
              "clearing the upload data page so the user can submit another meal");
          setState(() {
...
/*        setState(() {
        ...
        Config.log.i(
            "image clear button pressed, clearing the user's selected image");
        _imageFile = null;
        hideButton();
      },
    );
    ...
    if (_imageFileList != null) {
      ...
      return Container(
        ...
        itemCount: _imageFileList!
            .length, //this line prevents an error when loading the image, just keep it
      ));
    } else {
      return const Text(
        ...
        children: <Widget>[
          ...
          Row(children: <Widget>[
            Flexible(
                fit: FlexFit.tight,
                child: Container(
                  child: Visibility(visible: !_showButton, child: clearImage()),
                )),
            Flexible(
                fit: FlexFit.tight,
                child: Container(
                  child:
                      Visibility(visible: !_showButton, child: submitImage()),
                ))
          ])
        ],
      ),
