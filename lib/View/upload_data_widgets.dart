...
Widget addComments() {
  return const TextField(
    decoration:
        InputDecoration(border: OutlineInputBorder(), hintText: 'Comments'),
  );
}
...
Widget addWeight() {
  return const TextField(
    decoration:
        InputDecoration(border: OutlineInputBorder(), hintText: 'Weight'),
  );
}
...
Widget submitImage() {
  return ElevatedButton(
      ...
      onPressed: () {
        //TODO upload image and comments to db
      }
      // ,
      );
}
  ...
  if (_imageFileList != null) {
    ...
    return Container(
      ...
      itemCount: _imageFileList!.length
          as int, //this line prevents an error when loading the image, just keep it
    ));
  } else {
    return const Text(
