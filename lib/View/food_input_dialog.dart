...
class FoodInputDialog extends StatefulWidget {
  ...
  FoodInputDialog(
      this.currentInstitution, this.currentSubject, this.currentFoodStatus,
      {Key? key})
      : super(key: key);
...
class _FoodInputDialogState extends State<FoodInputDialog> {
  ...
  List<String> FoodItems1 = [
    ...
    "Sandwich",
    "Juice"
  ]; //for String foodItem in existingFoodItems for eaten status
  List<String> Selected1 =
      []; //items that get selected once are not suggested again for eaten status
  List<String> FoodItems2 = [
    "Apple",
    "Sandwich",
    "Juice"
  ]; //for String foodItem in existingFoodItems for container status
  List<String> Selected2 =
      []; //items that get selected once are not suggested again for container status
            ...
            child: Form(
              ...
              children: [
                ...
                newFoodSubmission
                    ? Column(children: [
                        suggestBox(
                            context, nameTextController, this._foodNameValid),
                        itemPresets(nameTextController)
                      ])
                    : Column(children: [
                        existingFoods(),
                        Visibility(
                            visible: !_previousFoodItemSelected,
                            child: Text(
                                "No food item selected: please select a food item",
                                style: TextStyle(
                ...
                foodWeightField(
                    weightTextController, "Weight(g)", this._foodWeightValid),
                foodInputField(commentsTextController, "Comments",
                    this._foodCommentsValid),
                submitData(),
                retakePhoto(),
              ],
            ))))));
  ...
  Widget existingFoods() {
    ...
    assert(widget.currentFoodStatus == FoodStatus.eaten ||
        widget.currentFoodStatus == FoodStatus.container);
            ...
            switch (snapshot.connectionState) {
              ...
              case ConnectionState.active:
                ...
                if (mealInfosSnapshot.value == null) {
                  ...
                  List<Widget> mealInfoList = mealInfoMap.values.map((element) {
                    ...
                    return ChoiceChip(
                      ...
                      onSelected: (bool selected) {
                        if (selected) {
                          Config.log.i(
                              "user has selected meal with name: ${currentMealInfo.name} and ID: ${currentMealInfo.mealId} to input new data for");
                          setState(() {
                  ...
                  return Wrap(
                      direction: Axis.horizontal, children: mealInfoList);
                }

              case ConnectionState.done:
                Config.log.i(
                    "connection state done when reading meal infos from the database");
  ...
  Future<image.Image> takeShot() async {
    ...
    image.Image capturedImage = image.decodePng(
        File(path!).readAsBytesSync())!; //encode the og image into IMG
    capturedImage = image.copyCrop(
        capturedImage, 5, 61, (width / 2).toInt() - 5, (height - 139).toInt());
    ...
    return ElevatedButton(
        onPressed: () {
          ...
          setState(() {
            ...
            this._foodWeightValid =
                inputWeight.isNotEmpty && inputWeight != null;
            ...
            this._previousFoodItemSelected =
                this.selectedExistingFoodItem != null;
          });

          if (widget.currentFoodStatus == FoodStatus.uneaten) {
            // we are submitting an entirely new food item
            if (this._foodNameValid &&
                this._foodWeightValid &&
                this._foodCommentsValid) {
              Config.log.i(
                  "submitting entirely new uneaten meal with name: $inputName, weight: $inputWeight, and comments: $inputComments");
            ...
            if (this._previousFoodItemSelected &&
                this._foodWeightValid &&
                this._foodCommentsValid) {
              // our meal submission is valid, submit our data
              Config.log.i(
                  "submitting new meal of status ${widget.currentFoodStatus.toString()} with existing name: ${this.selectedExistingFoodItem.name}, "
  ...
  Widget foodInputField(
      TextEditingController controller, String fieldName, bool fieldIsValid) {
