...
void main() {
  group("institution getters and setters", () {
    test("institution getters", () {
      final Institution testInstitution =
          Institution("test institution", "test address", 0);
      expect(testInstitution.name, "test institution");
      expect(testInstitution.address, "test address");

      final InstitutionInfo testInstitutionInfo =
          InstitutionInfo("test institution", "test address");
  ...
  group("institution operators", () {
    test("== operator", () {
      ...
      expect(
          testInstitution == Institution("test address", "test institution", 0),
          false);
      expect(testInstitution == Institution("test address", "test address", 0),
          false);
      expect(
          testInstitution ==
              Institution("test institution", "test institution", 0),
          false);
      expect(
          testInstitution == Institution("test institution", "test address", 0),
          true);
  ...
  group("institution json", () {
    test("to json", () {
      ...
      expect(testInstitution.toJson(), <String, dynamic>{
        "_name": "test institution",
    ...
    test("from json", () {
      final Map<String, dynamic> testJSON = json.decode(
          '{"_name":"test institution", "_address":"test address", "_subjectsMap":{}}');
      print(Institution.fromJSON(testJSON));
      print(Institution("test institution", "test address", 0));

      expect(Institution.fromJSON(testJSON),
          Institution("test institution", "test address", 0));
    });
  });
}
