...
void main() {
  ...
  group("institution info operators", () {
    ...
    test("== operator", () {
      ...
      expect(
          testInstitutionInfo == InstitutionInfo("something", "else"), false);
      expect(
          testInstitutionInfo ==
              InstitutionInfo("test address", "test institution"),
          false);
      expect(
          testInstitutionInfo ==
              InstitutionInfo("test address", "test address"),
          false);
      expect(
          testInstitutionInfo ==
              InstitutionInfo("test institution", "test institution"),
          false);
      expect(
          testInstitutionInfo ==
              InstitutionInfo("test institution", "test address"),
  ...
  group("json operations", () {
    test("to json", () {
      final InstitutionInfo testInstitutionInfo =
          InstitutionInfo("test institution", "test address");
      expect(testInstitutionInfo.toJson(), <String, dynamic>{
        "name": "test institution",
        "databaseKey": "test address",
    ...
    test("from json", () {
      ...
      expect(InstitutionInfo.fromJSON(testJSON),
          InstitutionInfo("test institution", "test address"));
    });
  });
}
